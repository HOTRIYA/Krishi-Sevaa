...
function themeFilter(darkMode, highContrast) {
  ...
  if (highContrast) parts.push("contrast(1.2) saturate(1.05)");
  return parts.length ? parts.join(" ") : "none";
}

function DarkModeToggle({ darkMode, setDarkMode, small }) {
  return (
    <button
      ...
      style={{
        ...
        width: small ? 30 : "auto", height: small ? 30 : "auto", padding: small ? 0 : "7px 12px",
        borderRadius: 8, border: `1px solid ${COLOR.border}`, background: COLOR.surface, cursor: "pointer",
        color: COLOR.textSecondary, fontSize: 12.5, fontWeight: 600,
      }}
    >
      <span key={darkMode ? "sun" : "moon"} className="ks-spin" style={{ display: "flex" }}>
        {darkMode ? <Sun size={14} /> : <Moon size={14} />}
      </span>
      {!small && (darkMode ? "Light" : "Dark")}
    </button>
  );
}
  ...
  return (
    <style>{`
      * { transition: background-color 0.35s ease, border-color 0.35s ease, color 0.35s ease, box-shadow 0.25s ease; }
      @keyframes ksFadeUp { from { opacity: 0; transform: translateY(8px); } to { opacity: 1; transform: translateY(0); } }
      @keyframes ksFadeIn { from { opacity: 0; } to { opacity: 1; } }
      @keyframes ksScaleIn { from { opacity: 0; transform: scale(0.94); } to { opacity: 1; transform: scale(1); } }
      @keyframes ksToastIn { from { opacity: 0; transform: translateY(14px) scale(0.97); } to { opacity: 1; transform: translateY(0) scale(1); } }
      @keyframes ksPulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.55; } }
      @keyframes ksSpinSlow { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
      @keyframes ksShimmer { 0% { background-position: -200px 0; } 100% { background-position: 200px 0; } }
      .ks-card { animation: ksFadeUp 0.4s cubic-bezier(.2,.7,.3,1) both; transition: transform 0.2s ease, box-shadow 0.25s ease, background-color 0.35s ease, border-color 0.35s ease; }
      .ks-card:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(20,30,20,0.08); }
      .ks-btn { transition: transform 0.15s ease, box-shadow 0.2s ease, filter 0.2s ease, background-color 0.25s ease, border-color 0.25s ease; }
      .ks-btn:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 5px 14px rgba(20,30,20,0.15); filter: brightness(1.04); }
      .ks-btn:active:not(:disabled) { transform: translateY(0) scale(0.96); }
      .ks-modal-backdrop { animation: ksFadeIn 0.2s ease both; }
      .ks-modal-box { animation: ksScaleIn 0.25s cubic-bezier(.2,.8,.3,1.2) both; }
      .ks-toast { animation: ksToastIn 0.35s cubic-bezier(.2,.8,.3,1.2) both; }
      .ks-pulse { animation: ksPulse 1.8s ease-in-out infinite; }
      .ks-spin { animation: ksSpinSlow 0.5s ease; }
      .ks-tab-btn { transition: all 0.25s cubic-bezier(.3,.7,.3,1); }
      .ks-tab-btn:hover { transform: translateY(-1px); }
      .ks-row { transition: transform 0.18s ease, box-shadow 0.2s ease, border-color 0.25s ease, background-color 0.3s ease; }
      .ks-row:hover { transform: translateX(2px); box-shadow: 0 3px 10px rgba(20,30,20,0.07); }
      ::selection { background: ${COLOR.forestTint}; color: ${COLOR.forestDeep}; }
    `}</style>
  );
}
...
function Card({ title, right, children, pad = 18, innerRef }) {
  return (
    <div ref={innerRef} className="ks-card" style={{ background: COLOR.surface, border: `1px solid ${COLOR.borderStrong}`, borderRadius: 12, padding: pad, marginBottom: 14 }}>
      {title && (
...
function Button({ children, onClick, variant = "secondary", small, disabled, icon: Icon }) {
  ...
  return (
    <button onClick={onClick} disabled={disabled} className="ks-btn" style={{ ...styles[variant], display: "inline-flex", alignItems: "center", gap: 6, padding: small ? "6px 12px" : "9px 16px", borderRadius: 7, fontSize: small ? 12.5 : 13.5, fontWeight: 600, cursor: disabled ? "not-allowed" : "pointer", opacity: disabled ? 0.5 : 1, whiteSpace: "nowrap" }}>
...
function EmptyState({ title, body }) {
  return (
    <div style={{ textAlign: "center", padding: "26px 10px", color: COLOR.textSecondary, animation: "ksFadeIn 0.4s ease both" }}>
...
function Modal({ title, onClose, children, width = 560 }) {
  return (
    <div className="ks-modal-backdrop" style={{ position: "fixed", inset: 0, background: "rgba(18,20,16,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 20 }} onClick={onClose}>
      <div className="ks-modal-box" style={{ background: COLOR.surface, borderRadius: 14, width, maxWidth: "100%", maxHeight: "88vh", overflowY: "auto", padding: 26 }} onClick={(e) => e.stopPropagation()}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
          <h2 style={{ margin: 0, fontSize: 16.5, fontWeight: 700 }}>{title}</h2>
          <button onClick={onClose} className="ks-btn" style={{ background: "none", border: "none", cursor: "pointer", color: COLOR.textMuted, borderRadius: 6 }}><X size={18} /></button>
        </div>
        {children}
      </div>
    </div>
...
function StatusDot({ status }) {
  ...
  return <span className={status === "Operational" ? "" : "ks-pulse"} style={{ width: 7, height: 7, borderRadius: "50%", background: c, display: "inline-block" }} />;
}
function BarList({ items, color = COLOR.forest, showPct, total }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      {items.map((item, idx) => (
        <div key={item.label} style={{ animation: `ksFadeUp 0.4s ease both`, animationDelay: `${idx * 0.05}s` }}>
          ...
          <div style={{ height: 6, borderRadius: 4, background: COLOR.surfaceSunken, overflow: "hidden" }}>
            <div style={{ height: "100%", width: `${(item.count / max) * 100}%`, background: color, borderRadius: 4, transition: "width 0.6s cubic-bezier(.2,.8,.3,1)" }} />
          </div>
        </div>
      ))}
    </div>
...
function GlobalToast({ msg }) {
  if (!msg) return null;
  return <div className="ks-toast" style={{ position: "fixed", bottom: 20, right: 20, background: COLOR.forestDeep, color: "#fff", padding: "10px 16px", borderRadius: 8, fontSize: 13, fontWeight: 600, zIndex: 300, boxShadow: "0 4px 16px rgba(0,0,0,0.25)" }}>✓ {msg}</div>;
}
...
const SYMPTOMS = [
  { id: "Fever", icon: "🔥", label: "Fever" }, { id: "Not eating", icon: "🚫", label: "Not eating" },
  { id: "Not drinking", icon: "💦", label: "Not drinking" }, { id: "Weak / inactive", icon: "💤", label: "Weak / inactive" },
  { id: "Breathing problem", icon: "🌬️", label: "Breathing problem" }, { id: "Diarrhea", icon: "⚡", label: "Diarrhea" },
...
function PublicHeader({ onSignInClick, darkMode, setDarkMode }) {
  return (
...
const ROLES = [
  ...
  { id: "surveillance", label: "Surveillance", sub: "Alerts, clusters & analytics", icon: "🛰️" },
  { id: "admin", label: "Admin", sub: "Users, rules & system config", icon: "🔧" },
];
...
function MobileAuthorityBlocked({ onBack, darkMode, setDarkMode }) {
  ...
  return (
    <div style={{ minHeight: "100dvh", background: COLOR.bg }}>
      ...
      <div style={{ padding: "24px 18px 90px" }}>
        <div style={{ background: COLOR.amberTint, border: `1px solid ${COLOR.amber}33`, borderRadius: 14, padding: 20, marginBottom: 20, animation: wobble ? "shake 0.5s ease-in-out" : "none" }}>
        ...
        <PrimaryBtn onClick={handleStartDesktop}><span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}><Monitor size={16} /> Start desktop mode</span></PrimaryBtn>
        <div style={{ textAlign: "center", marginTop: 14 }}>
      ...
      {localToast && (
        <div style={{ position: "fixed", bottom: 40, left: "50%", transform: "translateX(-50%)", background: COLOR.forest, color: "#fff", padding: "12px 18px", borderRadius: 8, fontSize: 13, fontWeight: 600, zIndex: 100000, animation: "slideUp 0.3s ease-out", boxShadow: "0 4px 12px rgba(0,0,0,0.15)", whiteSpace: "nowrap", pointerEvents: "none" }}>
          {localToast}
        </div>
      )}
    </div>
